...
if [ "$?" -ne "0" ]; then
  ...
  elif [ -f ./serial.no ] && [ -f ./license.key ]; then
    kubectl create secret generic -n $NAMESPACE $STACK --from-file=license=./license.key --from-file=serial=./serial.no
    if [ "$?" -ne "0" ]; then
      echo "Error creating secret from license key.  Fix error above and try again"
      exit 1
    fi
  else
